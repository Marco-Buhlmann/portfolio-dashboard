...
const transformAirtableData = (records) => {
  ...
  const holdings = records.map((record) => ({
    ...
    unitsSold: record.fields['Units Sold'] || null,
    redemptionPrice: parseFloat(record.fields['RP ($)']) || null,
...
const parseValuationHistory = (historyString) => {
  ...
  try {
    const result = historyString.split(',').map((entry) => {
    ...
    console.log('Parsed valuation history:', result);
    return result;
  } catch (e) {
    console.error('Error parsing valuation history:', e);
    return [];
  }
};
...
const buildFields = (data) => {
  ...
  // Number fields - send as integers
  const unitsHeld = parseInt(data.unitsHeld);
  if (!isNaN(unitsHeld) && unitsHeld > 0) fields['Units Held'] = unitsHeld;
  
  const sharesOutstanding = parseInt(data.sharesOutstanding);
